...
class MyBloc extends Bloc<MyEvent, MyState> {
  ...
  Future<void> _onFetchData(FetchDataEvent event, Emitter<MyState> emit) async {
    emit(
      DataLoadState(requestKey: event.key, statusState: StatusState.loading),
    );
    ...
    if (random == 0) {
      ...
      emit(
        DataLoadState(requestKey: event.key, statusState: StatusState.empty),
      );
    } else if (random == 2) {
      emit(
        DataLoadState(
  ...
  void fetchData() => add(FetchDataEvent());
}
